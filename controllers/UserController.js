...
async function _login(req, res, isAdmin) {
    try {
        const query = {
        ...
        res.status(200).json({ USER: USER_BODY });
...
export default {
    ...
    login: async(req,res) => {
        await _login(req, res, false);
    },
    login_admin: async(req,res) => {
        await _login(req, res, true);
    },
    ...
    update: async(req,res) => {
        try {
            ...
            const NUser = await models.User.findByIdAndUpdate({_id: req.body._id}, req.body, {
    ...
    remove: async(req,res) => {
        try {
            ...
            if (User.rol === 'instructor') {
                ...
                if (courseCount > 0) {
                    return res.status(200).json({ message: 403, message_text: 'No se puede eliminar el instructor porque tiene cursos asignados.' });
                }
            }
